const DATA_SOURCES = [
  ["./data/gefaesse.json", normalizeKnowledgeQuestions],
  ["./data/gefaesse_begriffe.json", normalizeTerms]
];
...
function normalizeTerms(terms) {
  return terms.map((term) => ({
    uid: `gefaesse-begriff-${term.id}`, subject: getSubjectForGroup(term.kategorie), group: term.kategorie,
...
function normalizeKnowledgeQuestions(questions) {
  return questions.map((question) => {
    ...
    return {
      uid: `gefaesse-${question.id}`, subject: getSubjectForGroup(question.thema), group: question.thema,
