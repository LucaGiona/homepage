...
- Screenshot für "Skulptur + Raum" in `projekte.html` aktualisiert (`assets/projekte/skulptur-raum.png`) — das externe Projekt hat ein neues Herobild bekommen (Commit "Neues Herobild eingefügt" im `raum_skulpturen`-Repo). Wie beim ersten Screenshot per lokalem PHP-Server + Headless-Chrome neu erzeugt und auf 1280×800 zugeschnitten.
- "Drink hinzufügen"-Button (`#toggle-add-drink`) in `html/drinks.html` von Braun auf Grün umgefärbt, damit er sich vom normalen Akzent-Braun (Filter-Button etc.) abhebt. Neues Token-Paar `--color-success`/`--color-success-hover` in `css/style.css` ergänzt, nach demselben Muster wie das bestehende `--color-danger`-Paar für die Löschen-Buttons.
- Jahreszeit-Checkboxen im "Eigene Drinks hinzufügen"-Formular auf ein 2-Spalten-Grid umgestellt (`.season-fieldset`): "Ganzes Jahr" steht allein oben (volle Breite), darunter feste Paare Frühling/Sommer und Herbst/Winter — keine Jahreszeit steht mehr für sich allein, unabhängig von der Viewport-Breite.
- Dabei einen latenten Bug gefunden und behoben: Die globale `input`-Regel in `css/style.css` (Breite 100 %, Padding, Border) griff versehentlich auch auf Checkboxen/Radios, wodurch deren Klickfläche/Flex-Verhalten verzerrt wurde — bei "Ganzes Jahr" (volle Grid-Breite) fiel das sichtbar auseinander (Checkbox und Text rissen auseinander). Regel auf `input:not([type="checkbox"]):not([type="radio"])` eingeschränkt; behebt das Problem auch für die bestehenden Alkohol-Radios, nicht nur für die neue Jahreszeit-Grid.
- Abstand zwischen Formularfeld und Hilfetext (`.help-text`, z. B. "Beispiel: Rum, Limette, Zucker" unter Zutaten) im "Eigene Drinks hinzufügen"-Formular vergrößert — vorher zog eine negative Top-Margin den Hilfetext eng ans Feld, jetzt `var(--space-2)` positiver Abstand. Betrifft alle Felder mit Hilfetext gleichermaßen (Zutaten, Rezeptur, Zubereitung, Garnitur, Tipp), da `.help-text` global gilt.
- Zwei Buttons im Drink-Formular farblich abgesetzt: "Abbrechen" (`#cancel-edit`) aus der neutralen Weiß-Gruppe (`#reset-filters`/`.edit-custom-drink`) herausgelöst und in leichtem Orange gestylt (neues Token-Paar `--color-warning`/`--color-warning-soft`/`--color-warning-hover`). "Meine Drinks löschen" (`#clearMyDrinks`) von `.delete-custom-drink` (dünner roter Outline) getrennt und auf ein solides Rot (`--color-danger`) umgestellt, damit die "alles löschen"-Aktion optisch deutlicher wirkt als die einzelnen Karten-Löschen-Buttons.
- Die drei Buttons am Ende des Drink-Formulars ("Drink speichern", "Abbrechen", "Meine Drinks löschen") in einen eigenen Container `<div class="form-actions">` gepackt. Abstand läuft jetzt über `gap` (Flexbox, mit `flex-wrap`) statt über die alte `.add-drink button { margin-right }`-Regel, die dafür entfernt wurde.
- "Filter anwenden"/"Zurücksetzen" standen auf Desktop nicht zuverlässig nebeneinander (waren lose Flex-Items in derselben Zeile wie die Select-Felder, Umbruch war unvorhersehbar je nach Breite). In `<div class="filter-actions">` gepackt mit `flex-basis: 100%`, damit sie immer als eigene Zeile unter den Select-Feldern stehen, innerhalb dieser Zeile aber zuverlässig nebeneinander mit `gap`. Mobile-Regel ergänzt, damit sie dort weiterhin volle Breite gestapelt bleiben.
- Zähler "X Drinks sind gespeichert" neben "Eigene Drinks hinzufügen:" ergänzt (Tablet/Desktop: nebeneinander; Mobile: bricht automatisch per `flex-wrap` darunter um, kein extra Breakpoint nötig). Neues eigenständiges Modul `js/drinksCount.js` (liest `customDrinks` aus LocalStorage, zählt, schreibt Text inkl. Singular/Plural-Unterscheidung — "1 Drink ist gespeichert" vs. "X Drinks sind gespeichert"); `role="status"` auf dem Element für Screenreader. `js/addDrink.js` ruft `updateDrinksCount()` nach Hinzufügen/Löschen/Alle-löschen auf, nach demselben lose gekoppelten `typeof fn === "function"`-Muster, das dort schon für Titel/Sortierung/Filter verwendet wird. Neue Klasse `.section-header` (Flex-Zeile, space-between) + `.drinks-count` (0.85rem, `font-weight: 400`, gedeckte Textfarbe — kleiner und nicht fett wie gewünscht).
